...
// Barangay data (2020 Census) - San Jose del Monte City, Bulacan
// Source: PSA (Philippine Statistics Authority)
// Note: SJDM has 62 barangays; showing top 20 for chart visualization
const barangayData = [
    { name: 'Muzon', pop: 127506 },
    { name: 'Gaya-Gaya', pop: 56896 },
    { name: 'Santo Cristo', pop: 49579 },
    { name: 'Minuyan Proper', pop: 46171 },
    { name: 'Kaypian', pop: 46045 },
    { name: 'Graceville', pop: 42207 },
    { name: 'Citrus', pop: 22893 },
    { name: 'San Pedro', pop: 16439 },
    { name: 'Tungkong Mangga', pop: 16324 },
    { name: 'Francisco Homes-Mulawin', pop: 11226 },
    { name: 'San Manuel', pop: 11432 },
    { name: 'Dulong Bayan', pop: 11008 },
    { name: 'Gumaoc West', pop: 8885 },
    { name: 'Francisco Homes-Narra', pop: 8293 },
    { name: 'San Rafael II', pop: 8139 },
    { name: 'Bagong Buhay I', pop: 7264 },
    { name: 'Francisco Homes-Guijo', pop: 6972 },
    { name: 'San Rafael IV', pop: 6447 },
    { name: 'Minuyan II', pop: 5977 },
    { name: 'Sapang Palay Proper', pop: 5804 }
];

// Historical data - San Jose del Monte City
// Source: PSA (Philippine Statistics Authority)
 ...
 * Data is loaded from competitive-index.json
 */
let cmciData = null;

/**
 * Load CMCI data from JSON file
 */
async function loadCMCIData() {
    try {
        const response = await fetch('../data/competitive-index.json');
        const jsonData = await response.json();

        // Transform JSON structure to match expected format
        cmciData = {
            years: jsonData.years.map(String),
            pillars: {},
            keyIndicators: {
                labels: jsonData.keyIndicators.map(indicator => indicator.name),
                data: jsonData.keyIndicators.map(indicator => indicator.values)
            }
        };

        // Transform pillars
        jsonData.pillars.forEach(pillar => {
            const camelCaseId = pillar.id.replace(/-([a-z])/g, (g) => g[1].toUpperCase());
            cmciData.pillars[camelCaseId] = {
                labels: pillar.indicators.map(ind => ind.name),
                data: pillar.indicators.map(ind => ind.values)
            };
        });

        return true;
    } catch (error) {
        console.error('Failed to load CMCI data:', error);
        // Fallback to default data if JSON fails to load
        initializeFallbackCMCIData();
        return false;
    }
}

/**
 * Fallback CMCI data in case JSON fails to load
 */
function initializeFallbackCMCIData() {
    cmciData = {
